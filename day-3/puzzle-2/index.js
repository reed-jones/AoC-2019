...
const generateGrid = num =>
  new Array(num)
    .fill()
    .map((_, y) =>
      new Array(num).fill().map(_ => ({ wire: 0, l: { "1": 0, "2": 0 } }))
    );

// const grid = generateGrid(2900); // 15000
const grid = {};
let pointer = { x: 0, y: 0 };
const offset = { x: 0, y: 0 }; // 5000, 6000
...
const mapWire = (wire, grid, flag, pointer) => {
  let count = 0;
  wire.forEach(p => {
    switch (p.direction) {
      case "U":
...
mapWire(wire_1, grid, 1, { ...pointer });
mapWire(wire_2, grid, 2, { ...pointer });

const intersections = Object.entries(grid)
  .flatMap(([y, allX]) =>
    Object.entries(allX).flatMap(([x, data]) => ({ y: +y, x: +x, ...data }))
  )
  .filter(i => i.wire === 3)
  .map(({ x, y, count }) => ({ x, y, t: Math.abs(x) + Math.abs(y), count }))
