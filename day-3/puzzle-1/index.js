...
mapWire(wire_1, grid, 1, { ...pointer }, { ...offset })
mapWire(wire_2, grid, 2, { ...pointer }, { ...offset })
// console.log(grid.length, grid[0].length);

// console.log(grid.map(a => a.map((b,x) => b === 0 ? '.' : b ).join('')).join("\n"))

console.log(
    grid.flatMap((a, y) => a.map((b, x) => b !== 3 ? 0 : Math.abs(x - offset.x) + Math.abs(y - offset.y)).filter(a => a)).sort((a,b) => a - b).find(_ => true)
)
// save index (x,y), todo: create add row/column before/after to expand the grid as needed
// const generateGrid = num => new Array(num)
//     .fill()
//     .map((_, y) => new Array(num)
//         .fill()
//         .map((_, x) => ({ x, y, wire: 0 })
//     ))

// console.table(generateGrid(3).map(a => a.map(t => t.wire)))

// todo drawGrid command which periodically draws the grid to the terminal
// (maybe clear and redraw to watch the wire grow ?)
